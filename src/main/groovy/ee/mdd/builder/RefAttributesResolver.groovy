...
class RefAttributesResolver {
	...
	RefHolder add(String name, Class type) {
		refHolders[name] = new RefHolder(name: name, type: type)
	}
	...
	def attributteDelegate = { FactoryBuilderSupport builder, node, Map attributes ->
		refHolders.each { name, RefHolder ref ->
			if (attributes.containsKey(name)) {
				ref.resolveOrStore(attributes[name], {  node[name] = it })
			}
		}
		refHolders.each { name, RefHolder ref -> attributes.remove(name) }
	}

	def postInstantiateDelegate = { FactoryBuilderSupport builder, Map attributes, Object node ->
		if(node instanceof Element) {
			refHolders.each { name, RefHolder ref ->
				if(ref.type.isInstance(node)) {
					ref.addResolved(node)
				}
			}
		}
	}
