...
#bookTable .${item.props[0].name}column span:hover {
	font-weight: bold;
	cursor: pointer;
}
''')
...
(function(){
	...
	app.controller("BookController", function(\\$scope) {
		...
		\\$scope.add = function(book) {
			if (!book.hasOwnProperty("${item.props[0].name}") && !book.hasOwnProperty("tag")) {
				book.${item.props[0].name} = book.tag = ++\\$scope.currentID;
			}
		...
		\\$scope.resetTo${item.props[0].name} = function() {
			\\$scope.books.forEach(function(book) {
				\\$scope.display${item.props[0].name}(book);
			});
		}
		...
		\\$scope.display${item.props[0].name} = function(book) {
			book.tag = book.${item.props[0].name};
		}
