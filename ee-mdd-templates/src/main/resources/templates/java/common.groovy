...
//  }
  ...
  items: { c -> c.model.findAllRecursiveDown( { Module.isInstance(it) }) },
  context: { c -> c.putAll( [ component: c.item.component, module: c.item.module] ) } ) {
    template('implInitializer', appendName:true,  body: '''<% if(item.name.equals('backend')) { %><% c.className = "${component.capShortName}InitializerImpl" %> ${macros.generate('implInitializer', c)}<% } %>''')
    template('initializer', appendName: true, body: '''<% if(item.name.equals('backend')) { %><% c.className = "${component.capShortName}InitializerBase" %> ${macros.generate('initializer', c)}<% } %>''')
    template('initializerWakeup', appendName: true, body: '''<% if(item.name.equals('backend')) { %><% c.className = "${component.capShortName}InitializerWakeup" %> ${macros.generate('initializerWakeup', c)}<% } %>''')
  }
