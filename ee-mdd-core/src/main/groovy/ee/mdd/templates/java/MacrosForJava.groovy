...
public enum $c.className {<% def last = item.literals.last(); item.literals.each { lit -> %>
  ...
  public boolean $lit.is {
    return this == $lit.underscored; 
      ...
      template('metaAttributes', body: '''<% def ret = ''; String newLine = System.properties['line.separator']; def annotations = c.item.metasForEntity; if(annotations) { annotations.each { ret += 'HAAALLLOOOO'+newLine+it.name } } else { ret = 'DAS OBJEKT IST LEIDER NULL' }%>
$ret''')
